...
// ================== UART2 环形缓冲区 ==================
uint8_t UART2_RxBuf[UART2_RX_BUF_SIZE];   // 环形缓冲区
...
void UART2_Init(uint32_t baudrate)
{
    ...
    printf("[UART2] 初始化成功，波特率=%d, 环形缓冲区=%d字节\r\n", baudrate, UART2_RX_BUF_SIZE);
}
...
{
    // 环形缓冲区由 GetRxData 自动推进 tail，无需手动清除
}

// ================== USART2 中断服务函数（环形缓冲区版） ==================
// 每收到一个字节，写入环形缓冲区
void USART2_IRQHandler(void)
{
    if (USART_GetITStatus(USART2, USART_IT_RXNE) == SET)
    {
        UART2_RxBuf[UART2_RxHead] = USART_ReceiveData(USART2);
